 ...
 * => →排除缩放的插件命令
 * ◎排除的字符即使缩放也不会改变大小
 * ◎如果设置排除放大倍率，缩放时尺寸将得到校正。
 * 2 代表 2 倍，0.5 代表 0.5 倍
 * ◎仅在缩小（拉动相机）时效果
 *
 *
 * 【基本3】使用“控制消息中的字符”进行相机操作
 * 消息期间
 ...
 * 【基本5】返回相机位置
 * => 插件命令 → 返回相机位置
 * ◎ 将相机的位置返回到感兴趣的目标中心。 换句话说，返回手动交错的部分
 * ◎ 应在滑动模式完成的同时执行。
 *
 *
 * [应用程序1]地图开始时更改相机
 * => 将每个插件命令的“启动时应用”设置为 true
 *
 *
 * [应用2] 留意你最喜欢的地方
 *创建一个特定于相机的事件，将其移动到您想要的任何点，并密切关注它
 *
 *
 * [应用3]带操作字符的运动控制
 * 在变焦放大结束时，移位 X 和移位 Y
 ...
 * [注1] 关于按名称指定字符
 * ◎ 当您输入一个单词时，
 * 在名称 + 备注字段中指定输入短语“包含”的事件
 * 对于“缩放”，名称 + 备注字段中包含缩放的事件
 * 输入 ◎ 播放器以指定播放器
 * 输入 ◎ Self 以指定运行事件本身。
 * ◎ 输入关注者 1 以指定第一个关注者。 如果跟随者 2，则第二个
 * ◎ 输入车辆 1 以指定第一辆车。 如果车辆 2
 * 输入 ◎v[***] 以使用变量。 v[5] 替换为变量 5
 * ◎、多个规格用分隔。 普里西亚、里德·奈良 普里西亚和里德
 *
 *
 * [注2] 关于按ID指定字符
 * ◎ 如果输入数值，事件 ID 将指定该值的事件。
 * 如果为 5，则为 ID 为 5 的事件
 * 输入 ◎-1 以指定播放器。
 * 输入 ◎0 以指定活动事件本身。
 * 输入 ◎-11 以指定第一个关注者。 如果为 -12，则第二个
 * 输入 ◎-101 以指定第一辆车。 如果为 -101，则第二个
 * 输入 ◎v[***] 以使用变量。 v[5] 替换为变量 5
 * ◎、多个规格用分隔。 5、7 如果为 5 和 7
 * 与 ◎ ~ 一起指定。 5~7 如果 5、6 和 7
 * 　
 *
 *
 * ◉ 利用規約 ◉
 ...
 * @arg 注視キャラ-ID
 * @text 凝视角色标识
 * @desc 按 ID 定位。 1：事件 ID。 0：事件本身，-1：玩家，-11到：关注者
 *
 * @arg 動作時間
 * @text 运行时间
 * @desc 摄像机工作时间。 5 帧 5，1 秒 1 秒。 使用 _2 执行两次，使用 _-1 循环。 带 W 的砝码
 *
 * @arg ズーム倍率
 * @text 变焦系数
 * @desc 目标缩放系数。 如果为 2，则相机与拍摄对象的距离是其两倍。 1 为空白。
 *
 * @arg ずらしX
 * @text 交错 X
 ...
 * @arg 対象キャラ-名前
 * @text 目标角色名称
 * @desc 按名称定位。 ：事件搜索，自我：事件本身，玩家：相同，全部：所有字符，v[***]：变量。 ，在 中指定了多个
 *
 * @arg 対象キャラ-ID
 * @text 目标字符 ID
 * @desc 按 ID 定位。 1：事件 ID。 0：事件本身，-1：玩家，-2：所有字符，v[***]：变量，多个，由 ~ 一起指定
 *
 * @arg 除外時拡大率
 * @text 排阻放大倍率
 * @desc 不包括缩放时的放大倍率。 1 为空白。 
 * @default 
 *
 * @arg 除外クリア
 * @text 排除清除
